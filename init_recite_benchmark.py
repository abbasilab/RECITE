...
from recite.benchmark.discovery import check_trial_versions_batch, get_chunked_nct_ids
...
def _completed_nct_ids(conn) -> Set[str]:
    """NCT IDs that have reached the end (recite or ec_changes with evidence)."""
    cursor = conn.cursor()
    try:
        rows = cursor.execute(
            "SELECT DISTINCT nct_id FROM recite"
        ).fetchall()
        from_recite = {row["nct_id"] for row in rows}
    except Exception:
        from_recite = set()
    try:
        rows = cursor.execute(
            """SELECT DISTINCT nct_id FROM ec_changes
               WHERE evidence_source_path IS NOT NULL AND evidence_source_path != ''"""
        ).fetchall()
        from_ec = {row["nct_id"] for row in rows}
    except Exception:
        from_ec = set()
...
def _stage2_completed(conn) -> Set[str]:
    ...
    cursor = conn.cursor()
    rows = cursor.execute("SELECT nct_id FROM trials_with_versions").fetchall()
...
def _stage3_completed(conn) -> Set[str]:
    ...
    cursor = conn.cursor()
    rows = cursor.execute("SELECT DISTINCT nct_id FROM trial_versions").fetchall()
    return {row["nct_id"] for row in rows}
...
def _stage6_completed_nct_ids(conn) -> Set[str]:
    """NCT IDs that have protocol text extracted (in protocol_texts table)."""
    cursor = conn.cursor()
    try:
        rows = cursor.execute(
            "SELECT nct_id FROM protocol_texts"
        ).fetchall()
        return {row["nct_id"] for row in rows}
...
def _get_stage_status(conn) -> Dict[str, Dict[str, Any]]:
    ...
    stage4_unique_trials = _safe_count("SELECT COUNT(DISTINCT nct_id) FROM ec_changes")
    # Stage 5: protocol check status
    stage5_total = stage4_unique_trials  # total trials to check
    stage5_checked = _safe_count("SELECT COUNT(DISTINCT nct_id) FROM ec_changes WHERE evidence_source IS NOT NULL")
    stage5_with_protocol = _safe_count(
        "SELECT COUNT(DISTINCT nct_id) FROM ec_changes WHERE evidence_source = 'protocol_pdf'"
    )
    ...
    try:
        stage6_remaining = _safe_count(
            """SELECT COUNT(DISTINCT ec.nct_id) FROM ec_changes ec
               WHERE ec.evidence_source = 'protocol_pdf'
               AND ec.nct_id NOT IN (SELECT nct_id FROM protocol_texts)"""
        )
    except Exception:
        pass
...
def _ids_for_chunks(
    ...
    """Return (chunk_index, nct_ids) for each requested chunk."""
    out = []
    for ci in chunk_indices:
        ids = get_chunked_nct_ids(all_nct_ids, ci, num_chunks, seed=chunk_seed)
        out.append((ci, ids))
    return out
...
def _build_batches(
    ...
    batches = []
    current = []
    for _ci, nct_ids in chunk_tuples:
        for nct_id in nct_ids:
            if nct_id in completed:
                continue
            current.append(nct_id)
                    ...
                    for attempt in range(max_retries + 1):
                        try:
                            ...
                            if stage_name == "Stage 2":
                                ...
                                extract_evidence(None, worker_conn, nct_ids=batch)
                            else:
...
def _get_nct_ids_from_source(
    ...
    if discovery_source == "trials_with_versions":
        cursor = conn.cursor()
        rows = cursor.execute(
            "SELECT nct_id FROM trials_with_versions ORDER BY checked_at DESC"
        ).fetchall()
...
def _run_discovery_if_needed(conn, discovery_method: str, max_trials: Optional[int]) -> List[str]:
    ...
    try:
        rows = cursor.execute(
            "SELECT nct_id FROM discovered_trials WHERE discovery_method = ? ORDER BY discovered_at",
            (discovery_method,),
        ).fetchall()
        existing = [row["nct_id"] for row in rows]
    except Exception:
        existing = []
    if existing:
        if max_trials:
    ...
    from recite.benchmark.discovery import discover_all_nct_ids
    count = 0
    for nct_id in discover_all_nct_ids(method=discovery_method, max_results=max_trials):
        cursor.execute(
            "INSERT OR IGNORE INTO discovered_trials (nct_id, discovery_method, discovered_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
            (nct_id, discovery_method),
        )
        count += 1
        if count % 1000 == 0:
            conn.commit()
...
def main(
    ...
    try:
        ...
        all_nct_ids = _get_nct_ids_from_source(conn, discovery_source, discovery_method, max_trials)
        if not all_nct_ids:
            logger.warning("No NCT IDs available. Exiting.")
            raise typer.Exit(1)

        completed = _completed_nct_ids(conn)
        chunk_tuples = _ids_for_chunks(all_nct_ids, chunk_indices, num_chunks, chunk_seed)
        ...
        if discovery_source != "discovered":
            ...
            chunk_tuples_s2 = _ids_for_chunks(all_nct_ids, chunk_indices, num_chunks, chunk_seed)
        ...
        chunk_tuples_s3 = _ids_for_chunks(all_nct_ids, chunk_indices, num_chunks, chunk_seed)
        ...
        if not batches_s3:
            ...
            if total_s3 < 100:
                ...
                ids_s3 = [nct_id for batch in batches_s3 for nct_id in batch]
        ...
        ec_change_nct_ids = [
            row["nct_id"] for row in conn.execute(
                "SELECT DISTINCT nct_id FROM ec_changes"
            ).fetchall()
        ]
        logger.info(f"Stage 5-6: Processing {len(ec_change_nct_ids):,} trials with EC changes (not all {len(all_nct_ids):,} discovered)")
        
        completed_s5 = _stage5_completed(conn)
        chunk_tuples_s5 = _ids_for_chunks(ec_change_nct_ids, chunk_indices, num_chunks, chunk_seed)
        ...
        completed_s6 = _stage6_completed_nct_ids(conn)
        chunk_tuples_s6 = _ids_for_chunks(ec_change_nct_ids, chunk_indices, num_chunks, chunk_seed)
