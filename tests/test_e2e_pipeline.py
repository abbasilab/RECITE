...
class TestEndToEnd:
    ...
    def test_full_pipeline_writes_db_and_parquet(self, tmp_path):
        ...
        for trial in TRIALS:
            ...
            assert result["has_change"] is True, f"Expected change for {trial['nct_id']}"
            diffs.append(result)
        ...
        rows = conn.execute("SELECT * FROM recite ORDER BY nct_id").fetchall()
        for row in rows:
        ...
        assert "preamended_text" in pq_table.column_names
        assert "evidence" in pq_table.column_names
        assert "amended_text" in pq_table.column_names
        ...
        assert set(bench_df["nct_id"].tolist()) == {"NCT00000001", "NCT00000002"}
