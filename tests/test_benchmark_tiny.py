...
from recite.benchmark.utils import clean_text, normalize_instance_id
...
class TestTextUtils:
    ...
    def test_normalize_instance_id_standard(self):
        assert normalize_instance_id("NCT00000001") == "NCT00000001"

    def test_normalize_instance_id_digits_only(self):
        assert normalize_instance_id("00000001") == "NCT00000001"

    def test_normalize_instance_id_with_spaces(self):
        assert normalize_instance_id(" NCT 00000001 ") == "NCT00000001"

    def test_normalize_instance_id_invalid(self):
        assert normalize_instance_id("INVALID") is None
...
class TestBuilder:
    ...
    def test_creates_instance_from_fixture(self, tmp_db, trial_record, protocol_text):
        ...
        assert len(rows) == 1
        row = rows[0]
        assert row["instance_id"] == "NCT00000001"
    ...
    def test_skips_duplicate(self, tmp_db, trial_record, protocol_text):
        ...
        tmp_db.execute(
            """INSERT INTO ec_changes
               (instance_id, source_version, target_version, ec_before, ec_after, evidence_source_path)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                trial_record["instance_id"],
                trial_record["source_version"],
    ...
    def test_empty_protocol_text_scores_zero(self, tmp_db, trial_record):
        ...
        tmp_db.execute(
            "INSERT INTO protocol_texts (instance_id, raw_text) VALUES (?, ?)",
            (trial_record["instance_id"], "short"),
        )
        tmp_db.commit()
