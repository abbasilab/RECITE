...
class TestDataProcessing:
    ...
    def test_pipeline_db_to_parquet(self, tmp_path):
        ...
        recite_rows = conn.execute("SELECT * FROM recite ORDER BY instance_id").fetchall()
        ...
        required_cols = {"instance_id", "source_text", "evidence", "reference_text",
                         "source_version", "target_version", "evidence_extraction_level"}
        ...
        assert set(df_back["instance_id"].tolist()) == {"NCT00000001", "NCT00000002"}
...
class TestPredictionGeneration:
    ...
    def test_mock_predictions_stored_correctly(self, tmp_path):
        ...
        for row in stored:
            ...
            assert row_d["source_text"] is not None
            assert row_d["reference_text"] is not None
