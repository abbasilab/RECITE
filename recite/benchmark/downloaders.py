...
def download_versions(
    ...
    If nct_ids is None, queries trials_with_versions table for candidates.
    
    Args:
        nct_ids: Optional list of specific NCT IDs to process
    ...
    if nct_ids is None:
        ...
        query = """
            SELECT nct_id, eligibility_version_numbers FROM trials_with_versions
        ...
        eligibility_data = {row["nct_id"]: row["eligibility_version_numbers"] for row in rows}
    else:
        if max_trials:
        ...
        if nct_ids:
            ...
            for i in range(0, len(nct_ids), batch_size):
                ...
                placeholders = ",".join("?" * len(batch_nct_ids))
                batch_rows = cursor.execute(
                    f"SELECT nct_id, eligibility_version_numbers FROM trials_with_versions WHERE nct_id IN ({placeholders})",
                    batch_nct_ids,
                ).fetchall()
                for row in batch_rows:
                    eligibility_data[row["nct_id"]] = row["eligibility_version_numbers"]
    ...
    for i, nct_id in enumerate(nct_ids, 1):
        ...
        if i % 10 == 0 or (total > 100 and i % (total // 10) == 0):
            ...
            logger.debug(f"Processing {nct_id} ({i}/{total})")
        
        # Check if already downloaded
        existing = cursor.execute(
            "SELECT COUNT(*) FROM trial_versions WHERE nct_id = ?", (nct_id,)
        ).fetchone()[0]
        
        if existing > 0:
            skipped += 1
            logger.debug(f"  Skipping {nct_id} (already in database)")
            # Track pending update (don't commit yet)
            pending_updates.append(nct_id)
            
            # Batch commit every commit_batch_size trials
            if len(pending_updates) >= commit_batch_size:
                for pending_nct_id in pending_updates:
                    cursor.execute(
                        "UPDATE trials_with_versions SET versions_downloaded = 1 WHERE nct_id = ?",
                        (pending_nct_id,),
                    )
                conn.commit()
        ...
        history_data = fetch_version_history(nct_id, adapter=adapter)
        
        if not history_data:
            failed += 1
            logger.warning(f"  Failed to fetch history for {nct_id}")
            continue
        ...
        if use_expedited:
            ...
            eligibility_versions_json = eligibility_data.get(nct_id)
            if eligibility_versions_json:
                try:
                    ...
                    if eligibility_versions:  # Only filter if we have eligibility versions
                        ...
                        logger.info(f"Downloading {len(versions)} versions for {nct_id} (expedited mode, skipped {skipped_versions})")
                        ...
                        logger.warning(f"  Empty eligibility_version_numbers for {nct_id}, downloading all versions (fallback)")
                        versions = all_versions
                except (json.JSONDecodeError, TypeError) as e:
                    logger.warning(f"  Failed to parse eligibility_version_numbers for {nct_id}: {e}, downloading all versions (fallback)")
                    versions = all_versions
            else:
                # No eligibility data, fallback to downloading all
                logger.warning(f"  No eligibility_version_numbers for {nct_id}, downloading all versions (fallback)")
                versions = all_versions
        else:
        ...
        for version_info in versions:
            ...
            # Fetch version data
            version_data = fetch_version_data(nct_id, version_num, adapter=adapter)
            
            if not version_data:
                logger.warning(f"  Failed to fetch version {version_num} for {nct_id}")
                continue
            ...
            cursor.execute(
                ...
                (nct_id, version_number, version_date, overall_status, eligibility_criteria, raw_data_json, module_labels)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    nct_id,
        ...
        downloaded += 1
        logger.debug(f"  Stored {len(versions)} versions for {nct_id}")
...
def check_protocol_availability(nct_id: str, adapter: ClinicalTrialsGovAdapter) -> dict:
    """
...
def download_protocols(
    ...
    If nct_ids is None, queries ec_changes table for trials that need checking.
    """
    ...
    if nct_ids:
        ...
        trials_to_process = nct_ids[:max_trials] if max_trials else nct_ids
    else:
        ...
        trials_to_process = [trial["nct_id"] for trial in trials]
    ...
    for i, nct_id in enumerate(trials_to_process, 1):
        ...
        if not availability.get("has_protocol"):
            ...
            conn.commit()
            logger.debug(f"    No protocol available for {nct_id}")
            continue
        
        # Protocol exists - try to download
        has_protocol += 1
        logger.debug(f"    Protocol exists for {nct_id}, downloading...")
        
        protocol_result = find_protocol_pdfs(nct_id, adapter, output_dir, protocol_info=availability.get("protocol_info"))
