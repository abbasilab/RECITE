...
def _create_trial_metadata_table(conn: sqlite3.Connection):
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS trial_metadata (
            ...
            FOREIGN KEY (nct_id) REFERENCES discovered_trials(nct_id)
        )
        """
    )
    conn.execute(
...
def _create_trial_versions_table(conn: sqlite3.Connection):
    ...
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_trial_versions_nct_id 
        ON trial_versions(nct_id)
        """
    )
...
def _create_protocol_texts_table(conn: sqlite3.Connection):
    ...
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_protocol_texts_nct_id
        ON protocol_texts(nct_id)
        """
    )


def _create_ec_changes_table(conn: sqlite3.Connection):
    """Create ec_changes table. Protocol text is in protocol_texts (keyed by nct_id)."""
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ec_changes (
            ...
            version_from_date TEXT,
            version_to_date TEXT,
            ...
            quality_score REAL,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            UNIQUE(nct_id, version_from, version_to)
        )
        """
    )
    ...
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_ec_changes_nct_id 
        ON ec_changes(nct_id)
        """
    )
    conn.execute(
        """
...
def _create_recite_table(conn: sqlite3.Connection):
    ...
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_recite_nct_id 
        ON recite(nct_id)
        """
    )
    conn.execute(
