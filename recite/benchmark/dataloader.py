...
def stream_from_db(
    ...
    try:
        ...
        for split_name in splits:
            ...
            # Build query (join on both instance_id and merge_source to avoid duplicates from merged DBs)
            base_query = """
                SELECT
                    r.id,
                    r.instance_id,
                    ...
                    tm.overall_status
                FROM recite r
                LEFT JOIN trial_metadata tm ON r.instance_id = tm.instance_id AND r.merge_source = tm.merge_source
            """
            if has_merge_source:
                if is_final_test:
                    query = base_query + """
