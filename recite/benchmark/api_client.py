...
def fetch_version_history(
    ...
    if method in ("auto", "internal_api"):
        try:
            ...
            logger.warning(f"Internal API failed for {nct_id}: {e}")
            if method != "auto":
                return None
    ...
    if method in ("auto", "v2_api"):
        try:
            ...
            logger.warning(f"V2 API failed for {nct_id}: {e}")
...
def fetch_version_data(
    ...
    if method in ("auto", "internal_api"):
        try:
            ...
            logger.warning(f"Internal API failed for {nct_id} version {version_number}: {e}")
            if method != "auto":
                return None
