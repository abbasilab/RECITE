...
class BenchmarkPrompts:
    ...
    def load(cls, config_path: Path) -> "BenchmarkPrompts":
        ...
        two_step = data.get("two_step_prompts")
        ...
        return cls(
            ...
            two_step_prompts=two_step if isinstance(two_step, dict) else None,
        ...
        try:
            if isinstance(model, dict):
                return model_callable(
                    preamended_text, evidence_cur,
                    version_from=version_from, version_to=version_to,
                )
            return model_callable(preamended_text, evidence_cur)
        except Exception as e:
            last_error = e
...
def _format_batched_judge_prompt(
    ...
    samples: list of dicts with 'amended_text' (ground_truth) and 'prediction'.
    """
    pairs_parts = []
    for i, s in enumerate(samples, 1):
        gt = (s.get("amended_text") or "").strip()
...
def run_single_sample(
    ...
    def _v(key: str, default: Any = None) -> Any:
        ...
        if val is None or (hasattr(pd, "isna") and pd.isna(val)):
            return default
        if key in ("id", "version_from", "version_to", "year") and val is not None:
            try:
    ...
    try:
        ...
        version_from = _v("version_from", 0)
        version_to = _v("version_to", 0)

        if use_two_step and is_endpoint and prompts.two_step_prompts:
        ...
        amended_text = sample_row.get("amended_text") or ""
        ...
        result = {
            ...
            "nct_id": str(sample_row.get("nct_id", "")),
            "version_from": version_from,
            "version_to": version_to,
            "preamended_text": preamended_text,
            "evidence": evidence,
    ...
    for split_name, df in splits.items():
            ...
            if existing_results:
                ...
                all_results[split_name] = {
                    ...
                    "metrics": {
                        ...
                        if metric not in [
                            "id", "nct_id", "llm_judge_raw_response",
                            "version_from", "version_to", "preamended_text", "evidence", "amended_text", "prediction",
                        ]
                    },
                }
        ...
        def _run_one_row(row_index: int) -> Tuple[PredictionRecord, Dict[str, Any]]:
            ...
            if use_two_step and is_endpoint:
                ...
                step2_user = tsp["step2_user_template"].format(
                    schema=schema_text,
                    version_from=int(row["version_from"]),
                    version_to=int(row["version_to"]),
                    preamended_text=row["preamended_text"],
                )
                step2_system = tsp.get("step2_system", "")
                prediction = _query_with_rag_retry(
                    system_prompt=step2_system,
                ...
                if isinstance(model, dict):
                    ...
                    prediction = model_callable(row["preamended_text"], row["evidence"])
            metadata = {
            ...
            pred_record = PredictionRecord(
                ...
                amended_text=row["amended_text"],
                prediction=prediction,
                timestamp=datetime.now().isoformat(),
                metadata=metadata if metadata else None,
            )
            default_metrics = default_eval_fn(row["amended_text"], prediction)
            if llm_judge_eval_fn is not None:
                llm_judge_metrics = llm_judge_eval_fn(row["amended_text"], prediction)
                combined_metrics = {**default_metrics, **llm_judge_metrics}
            else:
                combined_metrics = default_metrics
            result_dict = {
        ...
        with open(path) as f:
            for line in f:
                ...
                out.append(PredictionRecord(
                    ...
                    version_from=int(row.get("version_from", 0)),
                    version_to=int(row.get("version_to", 0)),
                    preamended_text=row.get("preamended_text", "") or "",
                    evidence=row.get("evidence", "") or "",
                    amended_text=row.get("amended_text", "") or "",
    ...
    with open(path, "a") as f:
        for pred in predictions:
            f.write(json.dumps({
                "id": pred.id,
                "nct_id": pred.nct_id,
                "version_from": pred.version_from,
                "version_to": pred.version_to,
                "preamended_text": pred.preamended_text,
                "evidence": pred.evidence,
                "amended_text": pred.amended_text,
