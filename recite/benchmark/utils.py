...
def normalize_nct_id(nct_id: str) -> Optional[str]:
    """
    Normalize NCT ID format.
    
    Args:
        nct_id: NCT ID in various formats
    ...
    nct_id = re.sub(r"^NCT\s*", "", nct_id)
    nct_id = re.sub(r"\s+", "", nct_id)
    
    # Validate format (8 digits)
    if re.match(r"^\d{8}$", nct_id):
        return f"NCT{nct_id}"
...
def extract_protocol_digits(nct_id: str) -> str:
    """
...
def get_trials_with_versions(
    ...
    cursor = conn.cursor()
    query = """
        SELECT nct_id FROM trials_with_versions
...
def should_skip_trial_version_check(
    ...
    dt_result = cursor.execute(
        "SELECT version_count FROM discovered_trials WHERE nct_id = ?",
        (nct_id,),
    ).fetchone()
    ...
    if not dt_result:
        twv_result = cursor.execute(
            "SELECT nct_id FROM trials_with_versions WHERE nct_id = ?",
            (nct_id,),
        ).fetchone()
    ...
    result = cursor.execute(
        ...
        SELECT 
            dt.version_count,
            CASE WHEN twv.nct_id IS NOT NULL THEN 1 ELSE 0 END as in_trials_with_versions
        FROM discovered_trials dt
        LEFT JOIN trials_with_versions twv ON dt.nct_id = twv.nct_id
        WHERE dt.nct_id = ?
        """,
        (nct_id,),
    ).fetchone()
