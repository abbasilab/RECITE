...
def discover_all_nct_ids_bulk_xml(
    ...
    if xml_files:
        ...
        for xml_file in xml_files:
            ...
            for nct_id in extract_nct_ids_from_xml(xml_file):
                yield nct_id
                count += 1
                if count % 10000 == 0:
...
def extract_nct_ids_from_xml(xml_path: Path) -> Iterator[str]:
    """
    ...
    try:
        ...
        # ClinicalTrials.gov XML uses <nct_id> tag
        # Try different possible namespaces
        import re
        
        # First try: direct nct_id tag (no namespace)
        nct_id_elements = root.findall(".//nct_id")
        for elem in nct_id_elements:
            nct_id = elem.text
...
def discover_all_nct_ids_via_api(
    max_results: Optional[int] = None,
) -> Iterator[str]:
    """
...
def check_trial_has_versions(
    ...
    logger.debug(f"  Checking version history for {nct_id}...")
    history_data = fetch_version_history(nct_id, adapter=adapter)
    
    if not history_data:
        logger.debug(f"  No version history found for {nct_id}")
    ...
    logger.debug(f"  Extracted moduleLabels for {nct_id}: {len(module_labels_dict)} versions")
    ...
    if use_expedited:
        ...
        if version_count >= 2 and not has_eligibility:
            logger.debug(f"  {nct_id}: {version_count} versions but no Eligibility in moduleLabels (filtered out)")
    else:
        has_multiple = version_count >= 2
    
    logger.debug(f"  {nct_id}: {version_count} versions, has_eligibility={has_eligibility}, has_multiple={has_multiple}")
    return has_multiple, version_count, has_eligibility, eligibility_versions, module_labels_dict


def get_chunked_nct_ids(
    nct_ids: List[str],
    chunk_index: int,
    total_chunks: int,
    seed: int = 42,
    ...
    Args:
        nct_ids: List of all NCT IDs to process
...
def check_trial_versions_batch(
    ...
    Args:
        nct_ids: List of NCT IDs to check
        adapter: Optional adapter instance
    ...
    if chunk_index is not None:
        ...
        nct_ids = get_chunked_nct_ids(nct_ids, chunk_index, total_chunks)
        logger.info(f"Processing chunk {chunk_index + 1}/{total_chunks} ({len(nct_ids):,} trials)")
        # Estimate time: ~4.79 trials/second (will be less if many are skipped)
        estimated_hours = len(nct_ids) / (4.79 * 3600)
    ...
    if conn is None:
        ...
        for nct_id in nct_ids:
            has_versions, count, has_elig, elig_versions, _ = check_trial_has_versions(
                nct_id, adapter, use_expedited=use_expedited
            )
            logger.debug(f"{nct_id}: {count} versions (has_multiple={has_versions}, has_eligibility={has_elig})")
        return
    ...
    for i, nct_id in enumerate(nct_ids, 1):
        ...
        should_skip = should_skip_trial_version_check(cursor, nct_id)
        ...
        try:
            ...
            try:
                ...
                cursor.execute(
                    ...
                    (nct_id, discovery_method, version_count, version_check_at)
                    VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                    """,
                    (nct_id, discovery_method, version_count),
                )
                ...
                if has_versions_flag:
                    ...
                    cursor.execute(
                        ...
                        (nct_id, version_count, checked_at, versions_downloaded,
                         has_eligibility_changes, eligibility_version_numbers, module_labels_json)
                        VALUES (?, ?, CURRENT_TIMESTAMP, 0, ?, ?, ?)
                        """,
                        (nct_id, version_count, bool(has_eligibility), eligibility_versions_json, module_labels_json_str),
                    )
                    logger.debug(f"  ✓ {nct_id}: {version_count} versions, has_eligibility={has_eligibility} (added to trials_with_versions)")
            ...
            failed += 1
            logger.warning(f"  ✗ Failed to check {nct_id}: {e}")
