...
def validate_pdf_version(
    ...
    Args:
        ...
        version_to: Target version number from EC change
        version_to_date: Target version date from EC change
        date_tolerance_days: Tolerance for date lag (default: 7 days)
        
    Returns:
        Tuple of (is_valid, match_type)
        - is_valid: True if PDF version ≤ version_to (no data leakage)
    ...
    # Match specific amendment date to version_to_date (with tolerance)
    pdf_date = pdf_version_info.get("pdf_date")
    amendment_dates = pdf_version_info.get("amendment_dates", [])
    
    if version_to_date and amendment_dates:
        version_date = _parse_date(version_to_date)
        if version_date:
            tolerance = timedelta(days=date_tolerance_days)
            # Check if any amendment date matches or is before version_to_date (with tolerance)
            for amendment_info in amendment_dates:
                ...
                if amendment_date_str:
                    ...
                    if amendment_date:
                        ...
                        # If amendment date <= version_to_date + tolerance, it's valid
                        # (allows small lag where PDF uploaded slightly after version)
                        if amendment_date <= (version_date + tolerance):
                            # Also check if this amendment number <= version_to (double check)
                            if amendment_num and amendment_num <= version_to:
                                return True, "date_based"
                        # If amendment date > version_to_date + tolerance, potential leakage
                        elif amendment_date > (version_date + tolerance):
                            logger.warning(
                                f"Amendment {amendment_num} date {amendment_date_str} > version_to_date {version_to_date} "
                                f"(exceeds {date_tolerance_days}-day tolerance)"
                            )
    
    # Fallback: Compare PDF date to version_to_date (with tolerance)
    if pdf_date and version_to_date:
        pdf_dt = _parse_date(pdf_date)
        version_dt = _parse_date(version_to_date)
        if pdf_dt and version_dt:
            ...
            if pdf_dt <= (version_dt + tolerance):
                ...
                logger.warning(
                    f"PDF date {pdf_date} > version_to_date {version_to_date} "
    ...
    if max_amendment is not None:
        if max_amendment <= version_to:
            return True, "amendment_number"
        else:
            logger.warning(
                f"PDF amendment {max_amendment} > version_to {version_to} - potential data leakage"
            )
...
def identify_amendments(
    ...
    query = """
        SELECT nct_id, COUNT(*) as version_count
        FROM trial_versions
        GROUP BY nct_id
    ...
    for i, trial in enumerate(trials, 1):
        ...
        if i % 10 == 0 or (total > 100 and i % (total // 10) == 0):
            ...
            logger.debug(f"  Processing {trial['nct_id']} ({i}/{total})")
        nct_id = trial["nct_id"]
        ...
        for i in range(len(versions) - 1):
            ...
            if change_result["has_change"]:
                ...
                existing = cursor.execute(
                    ...
                    WHERE nct_id = ? AND version_from = ? AND version_to = ?
                    """,
                    (nct_id, v1["version_number"], v2["version_number"]),
                ).fetchone()
                ...
                if not existing:
                    cursor.execute(
                        ...
                        (nct_id, version_from, version_to, version_from_date, version_to_date,
                    ...
                    changes_found += 1
                    logger.debug(
                        f"  Found EC change: {nct_id} v{v1['version_number']} -> v{v2['version_number']}"
                    )
...
def extract_protocol_texts(
    ...
    for i, row in enumerate(trials, 1):
        ...
        cursor.execute(
            """
            INSERT OR REPLACE INTO protocol_texts (nct_id, pdf_path, raw_text, pdf_date, page_count)
            VALUES (?, ?, ?, ?, ?)
            """,
            (nct_id, str(pdf_path), raw_text, pdf_date, page_count),
        )
        extracted += 1
    conn.commit()
...
def extract_evidence(
    ...
    Optional nct_ids or ec_change_ids restrict to a subset. When both are None,
    processes all trials with protocol PDFs not yet in protocol_texts.
    """
    # ec_change_ids: resolve to nct_ids for protocol_texts (we key by trial)
    if ec_change_ids is not None and len(ec_change_ids) > 0:
        cursor = conn.cursor()
        placeholders = ",".join("?" * len(ec_change_ids))
        rows = cursor.execute(
            f"SELECT DISTINCT nct_id FROM ec_changes WHERE id IN ({placeholders})",
            ec_change_ids,
        ).fetchall()
        nct_ids = [r["nct_id"] for r in rows]
    extract_protocol_texts(conn, nct_ids=nct_ids, max_trials=max_trials)
