...
def create_recite_instances(
    ...
    query = """
        SELECT ec.id, ec.nct_id, ec.version_from, ec.version_to,
               ec.ec_before, ec.ec_after, pt.raw_text AS protocol_raw_text
        FROM ec_changes ec
        LEFT JOIN protocol_texts pt ON ec.nct_id = pt.nct_id
    ...
    for i, change in enumerate(changes, 1):
        ...
        existing = cursor.execute(
            ...
            """,
            (change["nct_id"], change["version_from"], change["version_to"]),
        ).fetchone()
        
        if not existing:
            # Clean text for benchmark
            preamended_text = clean_text(change["ec_before"])
            amended_text = clean_text(change["ec_after"])
            ...
            if needs_explicit_id:
                cursor.execute(
                    """
                    INSERT INTO recite
                    (id, nct_id, version_from, version_to, preamended_text, evidence, amended_text, 
                ...
                cursor.execute(
                    """
                    INSERT INTO recite
                    (nct_id, version_from, version_to, preamended_text, evidence, amended_text, 
                     ec_change_id, evidence_extraction_level, evidence_extraction_score, merge_source)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
