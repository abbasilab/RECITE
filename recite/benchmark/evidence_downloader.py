...
def find_protocol_pdfs(
    ...
    https://clinicaltrials.gov/ProvidedDocs/{last_2_digits}/{instance_id}/{filename}
    ...
    # NCT04424641 -> 41
    last_2_digits = instance_id[-2:] if len(instance_id) >= 2 else instance_id
    ...
    base_url = f"https://clinicaltrials.gov/ProvidedDocs/{last_2_digits}/{instance_id}"
    ...
    for filename in filenames_to_try:
        ...
        try:
            ...
            result["error"] = str(e)
            logger.warning(f"Failed to download protocol PDF for {instance_id}: {e}")
...
def _download_protocol_pdf(
    ...
    return find_protocol_pdfs(instance_id, adapter, output_dir)
...
def _extract_api_evidence_fields(
    ...
    version_data = fetch_version_data(instance_id, version, adapter=adapter)
