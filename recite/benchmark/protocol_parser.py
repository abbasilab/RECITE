...
def match_ec_to_amendment(
    ...
    Args:
        ec_change: EC change from API (nct_id, version_from, version_to, ec_before, ec_after)
...
def filter_amendments_by_version(
    ...
    """
    Filter amendments to only include those available at version_to.
    
    Returns amendments that would have been known at the time of version_to,
    as if version_to is the latest version at that point in time.
    
    Args:
        amendment_table: List of amendment dictionaries
        version_to: Target version number
        version_to_date: Target version date
        date_tolerance_days: Tolerance for date lag (default: 7 days)
        
    Returns:
        Filtered list of amendments available at version_to
    """
    if not amendment_table:
        return []
    ...
    filtered = []
    version_date = parse_date(version_to_date) if version_to_date else None
...
def filter_raw_pdf_text_by_version(
    ...
    Identifies sections in raw text that correspond to amendments > version_to
    and removes/excludes them, leaving only content available at version_to.
    ...
    Returns:
        Filtered raw text containing only content up to version_to
    """
    if not raw_pdf_text or not amendment_table:
        return raw_pdf_text
    
    # Identify future amendments (> version_to)
    future_amendments = [
        a for a in amendment_table
        if a.get("amendment_number") and a.get("amendment_number") > version_to
    ]
    ...
    for future_num in future_amendment_nums:
        ...
        # Pattern: lines containing "Amendment N" where N > version_to
        ...
        for line in lines:
            ...
            if (
                ...
                or (re.search(rf"\b{future_num}\s*\([^)]+\)", line) and future_num > version_to)  # "3 (date)" anywhere
            ):
                # Skip this line
                continue
