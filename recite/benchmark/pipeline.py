...
def run_e2e_pipeline(
    ...
    try:
        ...
        if not force:
            existing_nct_ids = set(
                ...
                for row in cursor.execute(
                    "SELECT nct_id FROM discovered_trials WHERE discovery_method = ?",
                    (discovery_method,),
                ).fetchall()
            )
            if existing_nct_ids:
                logger.info(f"  Found {len(existing_nct_ids)} already discovered NCT IDs (will skip duplicates)")
        
        new_nct_ids_found = False
        for nct_id in discover_all_nct_ids(method=discovery_method, max_results=max_trials):
            ...
            if not force and nct_id in existing_nct_ids:
                logger.debug(f"  Skipping {nct_id} (already discovered)")
                continue
            ...
            cursor.execute(
                ...
                (nct_id, discovery_method, discovered_at)
            ...
            conn.commit()
            discovered_count += 1
            nct_ids_to_check.append(nct_id)
            existing_nct_ids.add(nct_id)
            new_nct_ids_found = True
            
            if max_trials and discovered_count >= max_trials:
                break
        
        # If no new NCT IDs were discovered but we have existing ones, use them
        if not new_nct_ids_found and existing_nct_ids and not force:
            logger.info(f"  All NCT IDs already discovered, using existing {len(existing_nct_ids)} NCT IDs")
            ...
            query = """
                SELECT nct_id FROM discovered_trials 
            ...
            nct_ids_to_check = [row["nct_id"] for row in rows]
            discovered_count = len(nct_ids_to_check)
        
        # If we need more and have existing ones (and not force mode), add them
        elif not force and max_trials and len(nct_ids_to_check) < max_trials:
            needed = max_trials - len(nct_ids_to_check)
            ...
            if nct_ids_to_check:
                ...
                candidate_ids = [row["nct_id"] for row in all_candidates]
                exclude_set = set(nct_ids_to_check)
                
                # Filter out excluded IDs
                filtered_ids = [nct_id for nct_id in candidate_ids if nct_id not in exclude_set]
                
                # Take only what we need
                nct_ids_to_check.extend(filtered_ids[:needed])
            else:
                ...
                nct_ids_to_check.extend([row["nct_id"] for row in rows])
        
        stats["discovered"] = len(nct_ids_to_check) if nct_ids_to_check else discovered_count
        logger.info(f"  Total NCT IDs available: {len(nct_ids_to_check) if nct_ids_to_check else discovered_count}")
        
        if not nct_ids_to_check:
            ...
            if os.getenv("SKIP_METADATA_IN_TESTS", "").lower() in ("1", "true", "yes"):
                ...
                if xml_cache_dir.exists() and any(xml_cache_dir.iterdir()):
                    ...
                    metadata_count = populate_trial_metadata_table(
                        conn, xml_cache_dir, nct_ids=nct_ids_to_check
                    )
        ...
        with_versions_count = cursor.execute(
            "SELECT COUNT(DISTINCT nct_id) FROM trials_with_versions"  # deduped across merge sources
        ).fetchone()[0]
        ...
        if trials_with_versions:
            for nct_id in trials_with_versions:
                cursor.execute(
                    """
        ...
        versions_downloaded_count = cursor.execute(
            "SELECT COUNT(DISTINCT nct_id) FROM trials_with_versions WHERE versions_downloaded = 1"  # deduped across merge sources
        ).fetchone()[0]
...
def get_pipeline_stats(conn: sqlite3.Connection) -> Dict[str, Any]:
    ...
    try:
        ...
        stats = {
            "discovered": cursor.execute("SELECT COUNT(DISTINCT nct_id) FROM discovered_trials").fetchone()[0],  # deduped across merge sources
            "with_versions": cursor.execute("SELECT COUNT(DISTINCT nct_id) FROM trials_with_versions").fetchone()[0],  # deduped across merge sources
            "versions_downloaded": cursor.execute(
                "SELECT COUNT(DISTINCT nct_id) FROM trials_with_versions WHERE versions_downloaded = 1"  # deduped across merge sources
            ).fetchone()[0],
            "with_ec_changes": cursor.execute(
                "SELECT COUNT(DISTINCT nct_id) FROM ec_changes"
            ).fetchone()[0],
            "with_protocols": cursor.execute(
                "SELECT COUNT(DISTINCT nct_id) FROM ec_changes WHERE evidence_source_path IS NOT NULL"
            ).fetchone()[0],
