...
def run_summary_phase(
    ...
    for row in rows:
        ...
        try:
            ...
            if summary:
                update_paper_trial_evidenced_summary(
                    accrual_db_path, psrc, pid, nct_id, summary
                )
                updated += 1
        except Exception as e:
            logger.warning(f"Summary phase failed for {psrc}/{pid} / {nct_id}: {e}")
