...
def update_paper_trial_evidenced_summary(
    ...
    try:
        conn.execute(
            ...
            WHERE paper_source = ? AND paper_source_id = ? AND trial_nct_id = ?
            """,
            (evidenced_summary, paper_source, paper_source_id, trial_nct_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def get_trial_metadata_enrollment(recite_db_path: Path, nct_id: str) -> Optional[int]:
    """Return enrollment for nct_id from recite.db trial_metadata, or None if missing."""
    ...
    try:
        row = conn.execute(
            "SELECT enrollment FROM trial_metadata WHERE nct_id = ?", (nct_id,)
        ).fetchone()
        if row is None:
            return None
