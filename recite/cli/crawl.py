...
def review_sync_paper_trials(
    ...
    if re_review_own:
        ...
        for match in completed_recent:
            try:
                ...
                logger.warning(f"Failed to restore match {match.get('paper_source_id')} <-> {match.get('trial_instance_id')}: {e}")
    ...
    for match in matches:
        try:
            ...
            trial = adapter.fetch_by_instance_id(match["trial_instance_id"])
            if not trial:
                logger.warning(f"Trial not found: {match['trial_instance_id']}")
                skipped += 1
                continue
            ...
            if score is None:
                logger.warning(f"Failed to evaluate match: {match['paper_source_id']} <-> {match['trial_instance_id']}")
                failed += 1
                continue
            ...
            if updated_success:
                updated += 1
                logger.info(
                    f"[{updated}] Updated match: {match['paper_source_id']} <-> {match['trial_instance_id']} "
                    f"(score={score.match_score}, applicability={score.applicability_score})"
                )
            else:
                logger.warning(f"Failed to update match: {match['paper_source_id']} <-> {match['trial_instance_id']}")
                failed += 1
        
        except Exception as e:
            logger.error(f"Error reviewing match {match.get('paper_source_id', 'unknown')} <-> {match.get('trial_instance_id', 'unknown')}: {e}")
            failed += 1
            continue
                ...
                for match in match_paper_to_trials(
                    ...
                    if is_high_quality_match(match, seed_threshold_match, seed_threshold_applicability):
                        ...
                        if seed_expansion:
                            ...
                            logger.debug(f"Queued match for seed expansion: {match.trial_instance_id}")
            ...
            for seed_match in seed_queue:
                try:
                    ...
                    if new_queries:
                        ...
                        logger.info(f"Seed expansion generated {len(new_queries)} queries from match {seed_match.trial_instance_id}")
                    ...
                    logger.warning(f"Seed expansion failed for match {seed_match.trial_instance_id}: {e}")
