...
@app.command()
def init_benchmark(
    nct_ids: Optional[List[str]] = typer.Argument(
        None,
    ...
    nct_ids_file: Optional[Path] = typer.Option(
        None, 
        "--nct-ids-file", 
    ...
    if nct_ids_file:
        logger.info(f"  - Manual NCT IDs file: {nct_ids_file}")
    if nct_ids:
        logger.info(f"  - Manual NCT IDs: {len(nct_ids)} IDs provided")
    if stop_after:
    ...
    if manual_nct_ids:
        ...
        if nct_ids_file:
            logger.info(f"  From file: {nct_ids_file}")
        from recite.benchmark.builders import create_recite_instances
...
def _load_nct_ids(nct_ids_file: Optional[Path], max_trials: Optional[int]) -> List[str]:
    ...
    if nct_ids_file and nct_ids_file.exists():
        ...
        logger.warning(
            f"No NCT IDs file provided. Using default sample of {len(nct_ids)} trials only. "
