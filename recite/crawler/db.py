...
def save_paper_trial_match(
    ...
    Args:
        ...
        trial_instance_id: NCT ID of the clinical trial
...
def update_paper_trial_match(
    ...
    if paper_title is None or trial_title is None:
        ...
        for m in existing_match:
            if m["trial_instance_id"] == trial_instance_id:
...
def has_paper_trial_match(
    ...
    with _get_conn() as conn:
        row = conn.execute(
            f"SELECT 1 FROM {table_name} WHERE paper_source=? AND paper_source_id=? AND trial_instance_id=?",
    ...
    with _get_conn() as conn:
        rows = conn.execute(f"""
            SELECT paper_source, paper_source_id, paper_title,
                   ...
                   created_at, updated_at
            FROM {table_name}
            WHERE trial_instance_id=? AND match_score >= ?
            ORDER BY match_score DESC, applicability_score DESC
        """, (trial_instance_id, min_match_score)).fetchall()
...
def get_paper_trial_matches_stats(model_preset: str) -> dict:
    ...
    with _get_conn() as conn:
        ...
        unique_trials = conn.execute(f"""
            SELECT COUNT(DISTINCT trial_instance_id) FROM {table_name}
        """).fetchone()[0]
...
def merge_matches_from_archive(model_preset: str) -> list[dict]:
    ...
    Returns:
        List of merged match dicts, deduplicated by (paper_source, paper_source_id, trial_instance_id)
    """
    return _merge_tables_from_archive(
